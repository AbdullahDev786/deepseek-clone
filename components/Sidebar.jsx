...
const Sidebar = ({expand, setExpand}) => {
  ...
  return (
    <div className={`flex flex-col justify-between bg-[#212327] pt-7 transition-all z-50 max-mmd:absolute max-md:h-screen ${expand ? 'p-4 w-64' : 'md:w-20 w-0  max-sm:overflow-hidden'}`}>
        <div>
            <div className={`flex ${expand ? "flex-row gap-10" : "flex-col items-center gap-8"}`}>
                ...
                <div onClick={() => expand ? setExpand(false) : setExpand(true)}
                 className='group relative flex items-center justify-center hover:bg-gray-500/20 transition-all duration-300 w-9 h-9 aspect-square rounded-lg cursor-pointer'>
